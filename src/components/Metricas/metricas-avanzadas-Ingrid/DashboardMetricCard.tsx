    ...
    return ( 
        <div className={`dashboard__metric relative ${className}`}> 
            ...
            {(selectedStartDate || selectedEndDate) && ( 
                <div className="dashboard__calendar-selected text-sm text-gray-600 mt-2"> 
                    {selectedStartDate && (
                        <p>Dia inicio:: {formatDate(selectedStartDate)}</p>
                    )}
                    {selectedEndDate && (
                        <p> Dia Final:: {formatDate(selectedEndDate)}</p>
                    )}
                </div> 
            )} 
        </div> 
